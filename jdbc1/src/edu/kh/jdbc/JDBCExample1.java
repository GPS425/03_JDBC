...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// java.sql.ResultSet
		// - SELECT 조회 수 결과(RESULT SET)를 저장하는 객체
		ResultSet rs = null;
		...
		try {
			...
			String userName = "kh_jjm"; 	// 사용자 계정명
			...
			// >> 결과로 int형을 반환(삽입, 수정, 삭제된 행의 개수)
			
			rs = stmt.executeQuery(sql);
			
			// 6. 조회결과가 담겨있는 rs를 커서(cursor) 이용하여 
			...
			// SQLException : DB연결과 관련된 모든 예외의 최상위 부모
			e.printStackTrace();
